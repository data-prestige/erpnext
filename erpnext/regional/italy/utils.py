...
@frappe.whitelist()
def export_invoices(zip_filename, quadratura, fattura, filters=None):
    frappe.has_permission("Sales Invoice", throw=True)
    attachments =[quadratura, fattura]
...
def prepare_and_attach_invoice(doc, replace=False):
    ...
    doc.node_id = frappe.db.get_value('Settings SDI', {'username': 'erpnext'}, ['node_id'])
...
def prepare_and_attach_quadratura(doc, invoice):
    progressive_name, progressive_number = get_progressive_name_and_number_sdi(doc, True)
    today = datetime.now()
    datestd = today.strftime('%Y%j')
    hourstd = today.strftime('%H%M') 
    
    progressive_number = "900"
    doc.node_id = progressive_name.split("_")[0]
    progressive_name = "FI."+progressive_name.split("_")[0] +"."+ \
        datestd +"."+ hourstd +"."+ progressive_number +".zip" 
        
    doc.created_time = datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]+"Z"
    doc.zip_filename = progressive_name
    doc.tipo_fatture = "FA"
    doc.numero_fatture = 1
    
    quadratura_xml = frappe.render_template(
        "erpnext/regional/italy/quadratura.xml",
        context={"doc": doc },
        is_path=True,
    )

    quadratura_xml = quadratura_xml.replace("&", "&amp;")
    xml_filename = progressive_name.replace(".zip", ".xml")
...
def sign_document(zip_file_path):
    from subprocess import call
    ## SIGN
    sign_cmd = f"openssl smime -sign -in {zip_file_path} -outform der -binary -nodetach -out {zip_file_path}.p7m -signer /home/sogei/CERTS/FIRMA.PEM -passin pass:Manager_182!"
    return call(sign_cmd, shell=True)

def encrypt_document(zip_file_path):
    from subprocess import call
    ## ENCRYPT
    encypt_cmd = f"openssl smime -encrypt -in {zip_file_path}.p7m -outform der -binary -aes256 -out {zip_file_path} /home/sogei/CERTS/sogeiunicocifra.pem"
    return call(encypt_cmd, shell=True)


# def decrypt_document(zip_file_path, output):
#     from subprocess import call
#     ## DECRYPT
#     sign_cmd = f"sudo openssl smime -decrypt -in /DatiDaSdITest/{zip_file_path}  -inform DER -binary -out {output} -recip /home/sogei/CERTS/CIFRA.PEM -passin pass:Manager_182!"
#     return call(sign_cmd, shell=True)

# def verify_document(zip_file_path, zip_filename):
#     from subprocess import call
#     ## VERIFY
#     encypt_cmd = f"openssl smime -verify -in {zip_file_path}.p7m -inform der -binary -out {output} -CAfile /home/sogei/CERTS/CAEntrate.pem"
#     return call(encypt_cmd, shell=True)

def create_sdi_support(zip_filename, attachments):
    import zipfile
    
    zip_file_path = frappe.utils.get_files_path(zip_filename, is_private=True)
    with zipfile.ZipFile(zip_file_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
        for file in attachments:
            file_path = frappe.utils.get_files_path(file.file_name, is_private=file.is_private)
            zip_file.write(file_path, arcname=file.file_name) 
    
    return zip_file_path, zip_file
    


@frappe.whitelist()
def send_data_sdi(docname):

    doc = frappe.get_doc("Sales Invoice", docname)
    frappe.has_permission("Sales Invoice", doc=doc, throw=True)

    quadratura = None
    for attachment in get_quadratura_attachments(doc):
        quadratura = attachment.file_name

    if quadratura:
        return "Already Sent!"

    
    atts = get_e_invoice_attachments(doc)
    filename=None
    ...
    if filename:
        ...
        e_invoice: "File" = frappe.get_doc("File", {"file_url": f"/private/files/{filename}"})
        quadratura = prepare_and_attach_quadratura(doc, e_invoice)
        zip_filename = quadratura.file_name.replace(".xml", ".zip")
        attachments = [e_invoice, quadratura]

        zip_file_path, zipfile = create_sdi_support(zip_filename, attachments) 
        sign_document(zip_file_path)
        encypted = encrypt_document(zip_file_path)

        ftp_put_files(zip_file_path)

        return "sdi support created"
    else:
        return "generate e-invoice"
    # download_zip(attachments, zip_filename)
    
def ftp_put_files(file):
    import pysftp
    cnopts = pysftp.CnOpts()
    cnopts.hostkeys = None
    with pysftp.Connection('35.214.251.81', username='erpnext', 
                            password='erpnext', cnopts=cnopts) as sftp:
        with sftp.cd('/DatiVersoSdITest'):   
            sftp.put(file)

      
@frappe.whitelist()
def check_sdi_status(docname):
    doc = frappe.get_doc("Sales Invoice", docname)
    frappe.has_permission("Sales Invoice", doc=doc, throw=True)
    atts = get_e_invoice_attachments(doc)
    for attachment in atts:
    ...
    if filename:
        try: 
            response = requests.get(f"http://localhost:8888/esito/filename/{filename}")
...
def get_quadratura_attachments(invoices):
    if not isinstance(invoices, list):
        if not invoices.company_tax_id:
            return
    ...
    for attachment in attachments:
        if (
            attachment.file_name
            and attachment.file_name.endswith(".xml")
            and attachment.file_name.startswith("FI")
        ):
            out.append(attachment)
...
def get_progressive_name_and_number(doc, replace=False):
    if replace:
        for attachment in get_e_invoice_attachments(doc):
            if not attachment.name.startswith("FI"):
                remove_file(attachment.name, attached_to_doctype=doc.doctype, attached_to_name=doc.name)
...
def get_progressive_name_and_number_sdi(doc, replace=False):
    if replace:
        for attachment in get_e_invoice_attachments(doc):
            if attachment.name.startswith("FI"):
                remove_file(attachment.name, attached_to_doctype=doc.doctype, attached_to_name=doc.name)
                filename = attachment.file_name.split(".xml")[0]
                return filename, filename.split("_")[1]

    node_id = frappe.db.get_value('Settings SDI', {'username': 'erpnext'}, ['node_id'])
    progressive_name = frappe.model.naming.make_autoname(node_id + "_.###")
    progressive_number = progressive_name.split("_")[1]
